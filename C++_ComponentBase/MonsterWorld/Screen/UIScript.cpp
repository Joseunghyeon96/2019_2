...
UIScript::UIScript(GameObject* gameObject)
	:Component(gameObject),ui(new char[UI_SIZE_X*UI_SIZE_Y])
{
...
void UIScript::start()
{	
	memset(ui, ' ', UI_SIZE_X * MAP_SIZE_Y);
	...
	gameObject->setScale(Vector2(UI_SIZE_X, MAP_SIZE_Y));
...
void UIScript::draw()
{
	Screen::getInstance().draw(gameObject->getShape().c_str()
